...
Android의 화면은 사용자의 동작이나 시스템 상태에 따라 생성되고, 화면에 표시되고, 백그라운드로 이동하거나 제거된다.

이러한 상태 변화를 Lifecycle이라고 한다.
...
Android에서는 주로 Activity와 Fragment의 생명주기를 이해해야 한다.
...
화면이 실제로 사용자와 상호작용 가능한 상태인지 판단할 때 중요한 생명주기다.

6. onPause()

Activity가 포커스를 잃었을 때 호출된다.

예를 들어 다른 Activity가 위에 나타나거나 다이얼로그가 표시될 수 있다.
...
예를 들어 세로 화면에서 가로 화면으로 변경하면 기존 Activity가 제거되고 새로운 Activity가 생성될 수 있다.
...
이 때문에 Activity에 데이터를 직접 저장하면 데이터가 사라질 수 있다.
...
Fragment에서 중요한 점은 Fragment 자체의 생명주기와 View의 생명주기가 다르다는 것이다.
...
여기가 Fragment에서 특히 중요하다.

Fragment 자체는 살아 있지만 View만 제거될 수 있기 때문이다.
...
따라서 View에 대한 참조를 계속 가지고 있으면 메모리 누수가 발생할 수 있다.
...
18. Activity와 Fragment 비교
구분	Activity	Fragment
생성	onCreate()	onAttach() → onCreate()
View 생성	onCreate() 내부	onCreateView()
View 초기화	onCreate()	onViewCreated()
화면 시작	onStart()	onStart()
사용자 상호작용	onResume()	onResume()
화면 종료	onPause()	onPause()
화면 비표시	onStop()	onStop()
View 제거	-	onDestroyView()
객체 제거	onDestroy()	onDestroy()
연결 해제	-	onDetach()
19. LifecycleOwner

Android에서는 생명주기를 직접 메서드 호출만으로 관리하지 않고 Lifecycle 객체를 통해 관찰할 수도 있다.

Activity와 Fragment는 LifecycleOwner를 구현한다.
...
가장 중요한 차이는 Fragment는 객체의 생명주기와 View의 생명주기가 분리되어 있다는 점이다. 특히 View Binding, Flow 수집, UI 이벤트 처리에서는 viewLifecycleOwner와 onDestroyView()를 함께 이해해야 한다.
